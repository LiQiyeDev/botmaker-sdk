...
public final class OcrPreprocessor {
    ...
    // --- BufferedImage <-> Mat bridge --------------------------------------------------------------

    /**
     * Copies a {@link BufferedImage} into a 3-channel BGR {@link Mat} — delegated to shared's
     * {@link OpencvManager}, which had a byte-identical copy of this.
     *
     * <p>The fork was deliberate once: the comment here read "re-implemented so shared does NOT depend on
     * the SDK", from when both halves lived in shared and the conversion could not be shared between them
     * without a dependency nobody wanted. Moving OCR into the SDK removed the reason, so the copy goes.
     */
    static Mat bufferedImageToMat(BufferedImage image) {
        return OpencvManager.bufferedImageToMat(image);
    }
