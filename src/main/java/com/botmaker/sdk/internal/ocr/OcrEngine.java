...
/**
 * On-screen text recognition — the OCR core behind the {@code Text} bot facade, which is its only caller.
 * Runs an OpenCV preprocessing pass ({@link OcrPreprocessor}) then Tesseract via Tess4J.
 *
 * <p>It lives under {@code internal} because a bot can only ever <em>receive</em> its results, through
 * {@code Text} — so it is uncatalogued by construction and the palette never offers it. What a bot does
 * write down by hand is {@code OcrOptions}, which is why that one is {@code api.vision}.
 *
...
public final class OcrEngine {
    ...
    public static List<TextResult> recognize(BufferedImage img, OcrOptions opts) {
        ...
        for (Word word : words) {
            ...
            // Tess4J speaks java.awt.Rectangle; the SDK speaks Rect, and the conversion stops here rather
            // than at every caller — which is what moving out of shared made possible.
