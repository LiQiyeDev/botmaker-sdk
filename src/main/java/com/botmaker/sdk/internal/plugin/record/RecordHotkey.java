...
/**
 * The recorder's <b>global</b> hotkey: one passive key watcher, live for as long as the recorder window is
 * open, that fires no matter which window has focus.
 *
 * <p>It has to be global. Recording exists to capture what you do <em>in the game</em>, so the game is what has
 * keyboard focus while a session runs — a JavaFX accelerator on the recorder's own scene would only ever fire
 * when the user had already clicked away from the thing they are recording, and clicking the Record button
 ...
 * <em>in</em> the recording — that is why {@link RecordingSession#ignoreKeysym} exists and why this class
 ...
 */
final class RecordHotkey implements AutoCloseable {
    ...
    RecordHotkey(Runnable onPressed) {
     ...
     * right there in the window, so a machine that won't give us a second XRecord client should lose the
     * shortcut, not the recorder.
     */
    void start() {
