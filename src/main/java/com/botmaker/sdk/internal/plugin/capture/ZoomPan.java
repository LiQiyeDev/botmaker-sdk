 ...
 *
 * <p>Extracted from Studio's {@code ObjectCaptureSurface}, which had the whole gesture inline, when
 * {@link ColorSampler} needed the same thing. Both surfaces show a frozen frame the user has to point at with
 * single-pixel accuracy, which is exactly the case where a 1:1 view is not enough — so the gesture has two
 * callers and belongs in one place rather than being spelled twice with slightly different clamps.
 *
 * <p><b>It has no vocabulary of its own</b> — no capture target, no colour, nothing of the SDK's API — so on
 * the shape-versus-table rule it belongs in {@code botmaker-plugin-toolkit} rather than here. It is here
 * because {@code ObjectCaptureSurface} is still Studio's for one more step, and Studio source may not name a
 * toolkit type. Move it to the toolkit when that surface follows.
 *
